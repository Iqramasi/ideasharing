...
function IdeaList() {
    ...
    console.log("IdeaList - API_BASE_URL:", API_BASE_URL); // Add this for debugging
    ...
    useEffect(() => {
        const fetchIdeas = async () => {
            ...
            setError(null);
            try {
                // Use the API_BASE_URL environment variable here!
                const res = await axios.get(`${API_BASE_URL}/api/ideas`);
                setIdeas(res.data);
            } catch (err) {
                console.error("Error fetching ideas:", err.response?.data?.message || err.message);
                setError("Failed to load ideas. Please try again later.");
            } finally {
                setLoading(false);
            }
    ...
    }, [refetchTrigger, API_BASE_URL]); // Add API_BASE_URL to dependency array

    useEffect(() => {
        if (location.state?.newIdeaSubmitted) {
            setRefetchTrigger(prev => prev + 1);
            // If using react-router-dom v6, you might want to clear the state:
    ...
    const handleShowInterest = async (e, ideaId) => {
        ...
        try {
            ...
            // Use the API_BASE_URL environment variable here too!
            await axios.post(`${API_BASE_URL}/api/ideas/${ideaId}/interest`);

        } catch (err) {
            console.error("Error registering interest:", err.response?.data?.message || err.message);
            setIdeas(prevIdeas =>
                prevIdeas.map(idea =>
                    idea._id === ideaId ? { ...idea, interestCount: Math.max(0, (idea.interestCount || 0) - 1) } : idea
                )
